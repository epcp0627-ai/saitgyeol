...
test("ships the finished editor without starter artifacts", async () => {
  ...
  assert.match(css, /\.preference-group[\s\S]*grid-template-columns: 6\.5em minmax\(0, 1fr\)/);
  assert.doesNotMatch(css, /\.friend-card:not\(\.ratio-portrait\) \.preference-group \.tag-block/);
